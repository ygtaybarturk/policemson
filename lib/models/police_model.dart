...
/// Gruplu poliçedeki bir kişi bilgisi
class GrupKisi {
  final String ad;
  final String soyad;
  ...
  factory GrupKisi.fromMap(Map<String, dynamic> m) => GrupKisi(
    ad: m['ad'] ?? '',
    soyad: m['soyad'] ?? '',
...
class Police {
  ...
  final List<GrupKisi>? grupKisileri; // Aynı poliçedeki diğer kişiler
  final int? kisiSayisi; // Grupta kaç kişi var
  ...
  int get kalanGun {
    ...
    final bitisBaslangic = DateTime(bitisTarihi.year, bitisTarihi.month, bitisTarihi.day);
    
    // Bitiş tarihi bugünden önce ise negatif değer dön
    if (bitisBaslangic.isBefore(bugunBaslangic)) {
      return -1; // Geçmiş
    }
    
    return bitisBaslangic.difference(bugunBaslangic).inDays;
  }
  ...
  Police copyWith({
    ...
    List<GrupKisi>? grupKisileri, int? kisiSayisi,
  }) => Police(
    id: id ?? this.id,
    ...
    grupKisileri: grupKisileri ?? this.grupKisileri,
  ...
  Map<String, dynamic> toMap() => {
    ...
    if (grupKisileri != null) 'grupKisileri': grupKisileri!.map((k) => k.toMap()).toList(),
  };
  ...
  factory Police.fromMap(Map<String, dynamic> m) => Police(
    ...
    grupKisileri: m['grupKisileri'] != null
        ? (m['grupKisileri'] as List?)?.map((k) => GrupKisi.fromMap(k as Map<String, dynamic>)).toList()
        : null,
  );
}
