...
class DatabaseService {
  ...
  Future<List<Police>> aylik(int y, int m, {PoliceStatus? filtre}) async {
    ...
    String w = 'bitis_tarihi BETWEEN ? AND ?';
    List a = [bas, bit];
    if (filtre != null) { w += ' AND durum=?'; a.add(filtre.index); }
    final r = await (await database).query('policeler', where: w, whereArgs: a, orderBy: 'bitis_tarihi ASC');
  ...
  Future<List<Map<String, dynamic>>> aralikAnaliz(int yil, int basMay, int bitMay) async {
    ...
    for (int m = basMay; m <= bitMay; m++) {
      ...
      final rows = await db.rawQuery('''
        ...
        FROM policeler WHERE bitis_tarihi BETWEEN ? AND ?
        GROUP BY durum
      ''', [bas, bit]);
  ...
  Future<Map<int, int>> aylikSayilar(int yil) async {
    ...
    for (int m = 1; m <= 12; m++) {
      ...
      final r = await db.rawQuery('SELECT COUNT(*) as c FROM policeler WHERE bitis_tarihi BETWEEN ? AND ?', [b, e]);
