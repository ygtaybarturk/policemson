...
class DatabaseService {
  ...
  Future<Database> _init() async {
    ...
    return openDatabase(
      path,
      version: 8,  // Version 8 - durum güncellemesi düzeltildi
      onCreate: (db, v) async {
      ...
      onUpgrade: (db, oldV, newV) async {
        ...
        if (oldV < 7) {
          // Version 7: Poliçe gruplama için veritabanını yeniden oluştur
          try {
            await db.execute('DROP TABLE IF EXISTS policeler');
            await db.execute('DROP TABLE IF EXISTS zeyiller');
          } catch (_) {}
          await _createTable(db);
          await _createZeyilTable(db);
        }
        if (oldV < 8) {
          // Version 8: Durum güncelleme düzeltmesi için yeniden oluştur
          try {
            await db.execute('DROP TABLE IF EXISTS policeler');
            await db.execute('DROP TABLE IF EXISTS zeyiller');
          } catch (_) {}
  ...
  Future<int> ekleVeyaGuncelle(Police p) async {
    ...
    if (mevcut.isNotEmpty) {
      ...
      final guncelMap = {
        ...
        'durum': p.durum.index,  // Durumu da güncelle
      };
