...
class DatabaseService {
  ...
  /// Akıllı ekleme: Aynı isimde müşteri varsa günceller, yoksa ekler
  ...
  Future<int> ekleVeyaGuncelle(Police p) async {
    ...
    var mevcut = await d.query('policeler',
        where: 'musteriAdi = ? AND soyadi = ? AND baslangicTarihi BETWEEN ? AND ?',
        whereArgs: [p.musteriAdi, p.soyadi ?? '', basAyBas, basAyBit],
        limit: 1);
    
    // Başlangıçta bulamadıysa bitiş tarihinde ara
    if (mevcut.isEmpty) {
      ...
      mevcut = await d.query('policeler',
          ...
          whereArgs: [p.musteriAdi, p.soyadi ?? '', bitAyBas, bitAyBit],
          limit: 1);
    }
