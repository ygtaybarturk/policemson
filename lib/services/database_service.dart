...
class DatabaseService {
  ...
  Future<int> ekleVeyaGuncelle(Police p) async {
    ...
    // Önce poliçe numarası ile ara (aynı poliçe no + aynı isim + aynı tür)
    List<Map<String, dynamic>> mevcut = [];
    
    if (p.policeNo != null && p.policeNo!.isNotEmpty) {
      // Poliçe numarası varsa önce onunla ara
      mevcut = await d.query('policeler',
          where: 'policeNo = ? AND musteriAdi = ? AND soyadi = ? AND tur = ?',
          whereArgs: [p.policeNo, p.musteriAdi, p.soyadi ?? '', p.tur.index],
          limit: 1);
    }
    
    // Poliçe numarası yoksa veya bulunamadıysa başlangıç tarihinde ara
    if (mevcut.isEmpty) {
  ...
  Future<List<Police>> policeNolaraGoreGetir(List<String> nolar) async {
    if (nolar.isEmpty) return [];
    final d = await db;
    final placeholders = nolar.map((_) => '?').join(',');
    final r = await d.query('policeler',
        where: 'policeNo IN ($placeholders)',
        whereArgs: nolar,
        orderBy: 'bitisTarihi ASC');
    return r.map(Police.fromMap).toList();
  }

  Future<List<Police>> adSoyadAra(String q) async {
    final d = await db;
    // Türkçe karakterleri normalize et (büyük harf + Türkçe→Latin)
    ...
    final qN = normalize(q);

    // Tüm poliçeleri çek, Dart tarafında filtrele (Türkçe LIKE SQLite'da çalışmaz)
    final r = await d.query('policeler', orderBy: 'bitisTarihi ASC');
    final tum = r.map(Police.fromMap).toList();

    return tum.where((p) {
      final ad     = normalize('${p.musteriAdi} ${p.soyadi ?? ''}');
      final plaka  = normalize(p.aracPlaka ?? '');
      final sirket = normalize(p.sirket);
      final no     = (p.policeNo ?? '').toUpperCase();
      return ad.contains(qN) || plaka.contains(qN) || sirket.contains(qN) || no.contains(qN);
    }).toList();
  }
  ...
  /// Tek aya ait özet
  Future<Map<String, dynamic>> aylikOzet(int yil, int ay) async {
    final liste = await aralikAnaliz(yil, ay, ay);
