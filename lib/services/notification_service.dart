...
class NotificationService {
  ...
  static const _hatirlaticiKanal = AndroidNotificationDetails(
    'policem_hatir_alarm', 'Müşteri Hatırlatıcıları',
    channelDescription: 'Poliçe ve takvim hatırlatıcıları',
    importance: Importance.max,
    priority: Priority.max,
    playSound: true,
    enableVibration: true,
    ...
    // Alarm kategorisi: sessiz modda bile çalar
    category: AndroidNotificationCategory.alarm,
    fullScreenIntent: true,
    visibility: NotificationVisibility.public,
  );
  ...
  void Function(String payload)? onBildirimTikla;

  Future<void> baslat({void Function(String payload)? onTikla}) async {
    tz_data.initializeTimeZones();
    tz.setLocalLocation(tz.getLocation('Europe/Istanbul'));
    if (onTikla != null) onBildirimTikla = onTikla;
    ...
    await _plugin.initialize(
      ...
      onDidReceiveNotificationResponse: (NotificationResponse res) {
        ...
        if (payload.isNotEmpty && onBildirimTikla != null) {
          onBildirimTikla!(payload);
        }
      },
    ...
    if (launchDetails?.didNotificationLaunchApp == true) {
      ...
      if (payload.isNotEmpty && onBildirimTikla != null) {
        // Biraz bekle ki Navigator hazır olsun
  ...
  Future<void> policeIcinBildirimler(Police police) async {
    ...
    // ── Kasko/Trafik: 14 gün önce, Diğerleri: 10 gün önce ──
    final erkenBildirimGun = (police.tur == PoliceType.kasko || police.tur == PoliceType.trafik) ? 14 : 10;
    final erkenBildirimOncesi = DateTime(
      bitis.year, bitis.month, bitis.day,
      9, 0, // Sabah 9:00
    ).subtract(Duration(days: erkenBildirimGun));

    if (erkenBildirimOncesi.isAfter(simdi)) {
      final mesaj = (police.tur == PoliceType.kasko || police.tur == PoliceType.trafik)
          ? '${police.tamAd} müşterisinin $turAdi poliçesi bitmesine $erkenBildirimGun gün kaldı. Fiyat teklifi gönder! 💰'
          : '${police.tamAd} müşterisinin $turAdi poliçesi bitmesine $erkenBildirimGun gün kaldı. Teklif hazırla! 📋';
      await _planla(
        id: _bid(police.id!, 90),
        baslik: '$turEmoji $turAdi Yenileme – $erkenBildirimGun Gün Kaldı!',
        icerik: mesaj,
        tarih: erkenBildirimOncesi,
        kanal: _normalKanal,
        payload: 'police:${police.id}',
      );
    }

    // ── 5 gün önceden hatırlatma ──
    if (kalanGun <= erkenBildirimGun) {
  ...
  // ── Public plugin erişimi (takvim iptal için) ─────────────
  FlutterLocalNotificationsPlugin get plugin => _plugin;

  // ── Takvim bağımsız hatırlatıcısı ────────────────────────
  Future<void> hatirlaticiPlanla({
    required int id,
    required String baslik,
    required String icerik,
    required DateTime tarih,
  }) async {
    if (tarih.isBefore(DateTime.now())) return;
    await _planla(
      id: id,
      baslik: baslik,
      icerik: icerik,
...
void _bgHandler(NotificationResponse res) {
  // Arka plan bildirimi — payload kayıt altında
}
