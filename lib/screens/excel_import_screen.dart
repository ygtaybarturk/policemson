...
class _ExcelImportScreenState extends State<ExcelImportScreen> {
  ...
  Future<void> _dosyaSec() async {
    ...
    try {
      ...
      for (int r = 1; r < rows.length; r++) {
        ...
        // Tutar
        final brutStr    = get(iBrut).replaceAll(',', '.').replaceAll(' ', '');
        final komisyonStr = get(iKomisyon).replaceAll(',', '.').replaceAll(' ', '');
        final tutar    = double.tryParse(brutStr)    ?? 0;
        final komisyon = double.tryParse(komisyonStr) ?? 0;
  ...
  DateTime? _tarihParse(String str, dynamic raw) {
    ...
    for (final f in formatlar) {
      try { return f.parseStrict(str.trim()); } catch (_) {}
    }
    return null;
  }
