...
class _PoliceDetayScreenState extends State<PoliceDetayScreen>
  ...
  void _doldur(Police p) {
    ...
    _komisyonCtrl   = TextEditingController(text: p.komisyon == 0 ? '' : p.komisyon.toStringAsFixed(0));
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Genel Notlar', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800)),
          IconButton(
            icon: Icon(_duzenleme ? Icons.save_outlined : Icons.edit_outlined),
            onPressed: _duzenleme ? _kaydet : () => setState(() => _duzenleme = true),
          ),
        ]),
        const SizedBox(height: 10),
        _duzenleme
          ? TextField(controller: _notlarCtrl, maxLines: 12,
              decoration: InputDecoration(hintText: 'Notlarınızı yazın…',
          ...
          : Container(
              ...
              child: Text(
                ...
                style: TextStyle(
                  color: _notlarCtrl.text.isEmpty ? Theme.of(context).colorScheme.onSurface.withOpacity(0.4) : null,
                  height: 1.6,
                ),
              ),
            ),
