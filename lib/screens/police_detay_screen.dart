...
class _PoliceDetayScreenState extends State<PoliceDetayScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_p != null && _p!.kisiSayisi != null && _p!.kisiSayisi! > 1) {
        setState(() {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            if (_p != null && _p!.kisiSayisi != null && _p!.kisiSayisi! > 1)
              Tab(icon: const Icon(Icons.people_outline), text: 'Kişiler (${_p!.kisiSayisi})'),
            const Tab(icon: Icon(Icons.notes_outlined), text: 'Notlar'),
          ],
        ),
      ),
      body: Column(children: [
        ...
        Expanded(child: TabBarView(controller: _tc, children: [
          _bilgiFormu(p),
          if (p.kisiSayisi != null && p.kisiSayisi! > 1) _grupKisileriSekme(p),
          _notlarSekme(),
        ])),
      ]),
    );
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (_duzenleme) ...[
          ...
          const SizedBox(height: 10),
          _tf(_belgeSeriCtrl, 'Belge Seri No (Kasko/Trafik)'),
          const SizedBox(height: 16),
          ...
          _infoKarti('📋 Poliçe', [
            ...
            if (p.policeNo != null && p.policeNo!.isNotEmpty) _satir('Poliçe No', p.policeNo!),
            if (p.belgeSeriNo != null && p.belgeSeriNo!.isNotEmpty) _satir('Belge Seri No', p.belgeSeriNo!),
          ]),
  ...
  Future<void> _policeSil(Police p) async {
    final onay = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Row(children: [
          ...
          SizedBox(width: 8),
          Text('Poliçeyi Sil', style: TextStyle(fontWeight: FontWeight.w900)),
        ]),
        content: Text(
          '${p.tamAd} adlı müşteriye ait ${p.goruntulenenTur} poliçesi kalıcı olarak silinecek.\n\nEmin misin?',
        ...
        actions: [
          ...
          FilledButton(
            ...
            child: const Text('Evet, Sil', style: TextStyle(fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    );
    ...
    await _db.sil(p.id!);
    if (!mounted) return;
    Navigator.pop(context, true); // true → listeye dön ve yenile
  }
  ...
  // ── TAB: Grup Kişileri ────────────────────────────────────
  Widget _grupKisileriSekme(Police p) {
    if (p.grupKisileri == null || p.grupKisileri!.isEmpty) {
      return const Center(child: Text('Kişi bilgisi bulunamadı'));
    }
    
    return SingleChildScrollView(
      padding: const EdgeInsets.all(14),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Toplam ${p.kisiSayisi} kişi',
          style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: kText)),
        const SizedBox(height: 12),
        
        ...List.generate(p.grupKisileri!.length, (i) {
          final kisi = p.grupKisileri![i];
          return Column(children: [
            _infoKarti('👤 Kişi ${i + 1}', [
              _satir('Ad Soyad', kisi.tamAd),
              if (kisi.tcKimlikNo != null && kisi.tcKimlikNo!.isNotEmpty)
                _satir('TC Kimlik', kisi.tcKimlikNo!),
            ]),
            if (i < p.grupKisileri!.length - 1) const SizedBox(height: 10),
          ]);
        }),
        const SizedBox(height: 20),
      ]),
