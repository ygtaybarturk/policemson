...
class _PolicelerScreenState extends State<PolicelerScreen>
  ...
  Future<void> _yukle() async {
    ...
    setState(() {
      final tumListe = r[0] as List<Police>;
      _liste     = _gruplaPoliceler(tumListe); // Gruplama ekledik
  ...
  // Aynı poliçe numarasına sahip poliçeleri grupla
  List<Police> _gruplaPoliceler(List<Police> liste) {
    if (liste.isEmpty) return [];
    
    // Poliçe numarasına göre grupla
    final Map<String, List<Police>> gruplar = {};
    
    for (final police in liste) {
      // Poliçe numarası yoksa veya boşsa tek başına göster
      if (police.policeNo == null || police.policeNo!.isEmpty) {
        final key = 'tek_${police.id}';
        gruplar[key] = [police];
        continue;
      }
      
      final policeNo = police.policeNo!;
      if (!gruplar.containsKey(policeNo)) {
        gruplar[policeNo] = [];
      }
      gruplar[policeNo]!.add(police);
    }
    ...
    for (final entry in gruplar.entries) {
      ...
      if (grup.length == 1) {
        ...
        // Birden fazla kişi - ilk kişiyi al, isimlerini birleştir
        final ilk = grup.first;
        final isimler = grup.map((p) => '${p.musteriAdi} ${p.soyadi}').join(', ');
        
        // Toplam tutar ve komisyon
        final toplamTutar = grup.fold(0.0, (sum, p) => sum + p.tutar);
        final toplamKomisyon = grup.fold(0.0, (sum, p) => sum + p.komisyon);
        
        // Yeni gruplanmış poliçe oluştur
        final gruplananPolice = Police(
          id: ilk.id,
          musteriAdi: isimler,
          soyadi: '',
          telefon: ilk.telefon,
          tcKimlikNo: ilk.tcKimlikNo,
          sirket: ilk.sirket,
          tur: ilk.tur,
          policeNo: ilk.policeNo,
          belgeSeriNo: ilk.belgeSeriNo,
          ruhsatSeriNo: ilk.ruhsatSeriNo,
          aracPlaka: ilk.aracPlaka,
          baslangicTarihi: ilk.baslangicTarihi,
          bitisTarihi: ilk.bitisTarihi,
          tutar: toplamTutar,
          komisyon: toplamKomisyon,
          durum: ilk.durum,
          olusturmaTarihi: ilk.olusturmaTarihi,
          hatirlaticiTarihi: ilk.hatirlaticiTarihi,
          hatirlaticiNotu: ilk.hatirlaticiNotu,
          notlar: ilk.notlar,
          email: ilk.email,
          dogumTarihi: ilk.dogumTarihi,
          aracMarka: ilk.aracMarka,
          aracModel: ilk.aracModel,
          aracYil: ilk.aracYil,
          ozelTurAdi: ilk.ozelTurAdi,
          adres: ilk.adres,
          uavt: ilk.uavt,
          pdfDosyaYolu: ilk.pdfDosyaYolu,
          yenilemeStatus: ilk.yenilemeStatus,
        );
        
        sonuc.add(gruplananPolice);
      }
    }
  ...
  Future<void> _aramaYap() async {
    ...
    setState(() { 
      _aramaListesi = _gruplaPoliceler(r); // Arama sonuçlarını da grupla
      _aramaYukl = false; 
    });
  }
            ...
            Stack(children: [
              Container(
                ...
                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    // İSİM SOYİSİM – büyük ve belirgin
                    Text(
