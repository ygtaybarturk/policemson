...
class _PolicelerScreenState extends State<PolicelerScreen>
  ...
  Future<void> _yukle() async {
    ...
    setState(() { _liste = r[0] as List<Police>; _sayilar = r[1] as Map<int,int>; _yukl = false; });
  }
  ...
  Future<void> _dahaSonraTakvim(Police p) async {
    await Navigator.push(context, _slide(TakvimScreen(
      policeId: p.id,
      onHatirlaticiSec: (t, n, g) async {
        await _notif.policeIcinIptal(p.id!);
        await _notif.dahaSonraHatirlatici(g);
        _yukle();
      },
    )));
    _yukle();
  ...
  Widget _anaEkran() => Column(children: [
    ...
    Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text('${_aylar[_ay]} $_yil',
      ...
      child: _yukl
          ...
          : _liste.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (_, i) => _AnimCard(
                    delay: Duration(milliseconds: (i * 55).clamp(0, 400)),
                    child: _PoliceKarti(
                      police: _liste[i],
                      onTap: () async {
...
class _AnimCard extends StatefulWidget {
  ...
  @override State<_AnimCard> createState() => _AnimCardState();
}
class _AnimCardState extends State<_AnimCard> with SingleTickerProviderStateMixin {
  ...
  late final Animation<Offset> _slide = Tween(begin: const Offset(0, 0.05), end: Offset.zero)
      .animate(CurvedAnimation(parent: _c, curve: Curves.easeOutCubic));
  @override
  void initState() { super.initState(); Future.delayed(widget.delay, () { if (mounted) _c.forward(); }); }
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      _SheetTile(icon: Icons.table_chart_rounded, bg: const Color(0xFFEAF8F0), iconColor: Color(0xFF00955A),
          title: "Excel'den Toplu Aktar", sub: 'Üretim listesini yükle', onTap: onExcel),
...
class _PoliceKarti extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final moneyFmt    = NumberFormat.currency(locale: 'tr', symbol: '₺', decimalDigits: 0);
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                // Belge No (varsa)
                if (p.belgeSeriNo != null && p.belgeSeriNo!.isNotEmpty) ...[
                  Row(children: [
                    Icon(Icons.confirmation_number_outlined, size: 12, color: kTextSub),
                    const SizedBox(width: 5),
                    Text('Belge No: ${p.belgeSeriNo!}',
                ...
                Container(
                  ...
                  child: Column(children: [
                    if (showBtn) ...[
                      Row(children: [
                        Expanded(child: _ActBtn('Yapıldı',    Icons.check_circle_outline_rounded, kSuccess, onYapildi)),
                        const SizedBox(width: 6),
                        Expanded(child: _ActBtn('Yapılamadı', Icons.cancel_outlined,              kDanger,  onYapilamadi)),
                        const SizedBox(width: 6),
                        Expanded(child: _ActBtn('Sonra',      Icons.watch_later_outlined,         kWarn,    onDahaSonra)),
                      ]),
                      const SizedBox(height: 8),
                    ],
...
class _HeaderBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, bg) = switch (durum) {
      ...
      _ => kalanGun <= 0   ? ('🔴 Bugün',        Colors.red.withOpacity(0.5))
...
class _ActBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => OutlinedButton.icon(
    ...
    label: Text(label, style: const TextStyle(fontSize: 10.5, letterSpacing: -0.2)),
