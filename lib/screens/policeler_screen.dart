...
class _PolicelerScreenState extends State<PolicelerScreen>
  ...
  Widget _buildFab() => Container(
    decoration: BoxDecoration(
      ...
      boxShadow: [
        BoxShadow(color: kPrimary.withOpacity(0.25), blurRadius: 12, offset: const Offset(0, 4)),
      ],
    ),
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: on ? [BoxShadow(color: kPrimary.withOpacity(0.20), blurRadius: 4, offset: const Offset(0,2))] : [],
                ),
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: on ? [BoxShadow(color: kPrimary.withOpacity(0.15), blurRadius: 4, offset: const Offset(0,1))] : [],
                ),
    ...
    Expanded(
      child: _yukl
          ...
          : _liste.isEmpty && _zeyiller.isEmpty
              ? _bosEkran()
              : ListView(
                  padding: const EdgeInsets.fromLTRB(11, 4, 11, 95),
                  cacheExtent: 400,
      ...
      child: GestureDetector(
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: acil ? kDanger.withOpacity(0.08) : accent.withOpacity(0.06),
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Row(children: [
                      ...
                      Flexible(child: Text(p.sirket,
                          style: TextStyle(fontSize: 9.5, color: Colors.white.withOpacity(0.82), fontWeight: FontWeight.w600),
                          overflow: TextOverflow.ellipsis)),
                      if (bireySayisi > 1) ...[
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (p.policeNo != null && p.policeNo!.isNotEmpty) ...[
                  Row(children: [
                    const Icon(Icons.numbers_rounded, size: 10, color: kTextSub),
                    const SizedBox(width: 4),
                ...
                if (p.aracPlaka != null && p.aracPlaka!.isNotEmpty) ...[
                  Row(children: [
                    Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        Icon(Icons.directions_car_rounded, size: 10, color: accent),
                        const SizedBox(width: 3),
                        Text(p.aracPlaka!, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: accent)),
                      ]),
                    ),
                ...
                Container(
                  ...
                  child: Row(children: [
                    // Başlangıç
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Başl.', style: const TextStyle(fontSize: 8.5, color: kTextHint, fontWeight: FontWeight.w700)),
                      const SizedBox(height: 2),
                      Text(fmt.format(p.baslangicTarihi),
                          style: const TextStyle(fontSize: 10, color: kText, fontWeight: FontWeight.w700)),
                    ])),
                    // Ok
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: const Icon(Icons.arrow_forward_rounded, size: 9, color: kTextSub),
                      ),
                    ),
                    // Bitiş
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                ...
                Row(children: [
                  ...
                  if (isYapildi)
                    Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(Icons.check_circle_rounded, size: 10, color: kSuccess),
                        const SizedBox(width: 3),
                        const Text('Yapıldı',
                            style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: kSuccess)),
                      ]),
                    ),
                  if (p.durum == PoliceStatus.yapilamadi)
                    Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(Icons.cancel_rounded, size: 10, color: kDanger),
                        const SizedBox(width: 3),
                        const Text('Yapılamadı',
                            style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: kDanger)),
                      ]),
                    ),
                  if (p.durum == PoliceStatus.dahaSonra)
                    Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(Icons.watch_later_rounded, size: 10, color: kWarn),
                        const SizedBox(width: 3),
                        const Text('Daha Sonra',
                            style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: kWarn)),
                      ]),
                    ),
                ]),

                // Prim tutarı + Hatırlatıcı Butonu (Turuncu, Sağ Taraf)
                if (toplamTutar > 0) ...[
                  const SizedBox(height: 6),
                  Row(children: [
                    // SOL: Prim bilgisi
                    Expanded(
                      child: Row(children: [
                        const Icon(Icons.payments_outlined, size: 10, color: kTextSub),
                        const SizedBox(width: 4),
                        Text('Prim: ', style: const TextStyle(fontSize: 9.5, color: kTextSub)),
                        Text(moneyFmt.format(toplamTutar),
                            style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: accent)),
                        if (toplamKomisyon > 0) ...[
                          const SizedBox(width: 4),
                          Text('• K: ${moneyFmt.format(toplamKomisyon)}',
                              style: const TextStyle(fontSize: 8.5, color: kTextSub)),
                        ],
                      ]),
                    ),
                    // SAĞ: Hatırlatıcı Butonu (Turuncu)
                    if (p.hatirlaticiTarihi != null &&
                        p.hatirlaticiTarihi!.isAfter(DateTime.now()))
                      GestureDetector(
                        onTap: widget.onDahaSonra,
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                          decoration: BoxDecoration(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            const Icon(Icons.alarm_rounded, size: 12, color: Color(0xFFFF9500)),
                            const SizedBox(width: 3),
                            Text(DateFormat('d MMM HH:mm', 'tr').format(p.hatirlaticiTarihi!),
                                style: const TextStyle(fontSize: 8.5, color: Color(0xFFFF9500), fontWeight: FontWeight.w700)),
                          ]),
                        ),
                      ),
                  ]),
  ...
  Widget build(BuildContext context) {
    if (yapildi) {
      return Container(
        ...
        child: Text(kalanGun > 0 ? '📅 $kalanGun gün' : '🏁 Bitti',
            style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: kSuccess)),
      );
    }
    final (t, bg, fg) = kalanGun < 0
        ? ('🔴 Bitti', kDanger.withOpacity(0.10), kDanger)
        : kalanGun == 0
        ? ('🔴 Bugün!', kDanger.withOpacity(0.10), kDanger)
        : kalanGun <= 3
        ? ('🔴 $kalanGun gün', kDanger.withOpacity(0.10), kDanger)
        : kalanGun <= 10
        ? ('⚠️ $kalanGun gün', kWarn.withOpacity(0.10), kWarn)
        : ('📅 $kalanGun gün', kPrimary.withOpacity(0.08), kPrimary);
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
      decoration: BoxDecoration(
      ...
      child: Text(t, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: fg)),
    );
  }
}
...
class _ActBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => OutlinedButton.icon(
    onPressed: onTap,
    icon: Icon(secili ? _doluIcon(icon) : icon, size: 10),
    label: Text(label, style: TextStyle(fontSize: 9, letterSpacing: -0.2, fontWeight: secili ? FontWeight.w800 : FontWeight.w600)),
    style: OutlinedButton.styleFrom(
