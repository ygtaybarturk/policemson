...
class _Satir {
  ...
  Police toPolice() {
    ...
    return Police(
      ...
      belgeSeriNo:     null,           // Belge Seri → ruhsatSeriNo'ya gider
      ruhsatSeriNo:    belgeSeriNo.isEmpty ? null : belgeSeriNo,
...
class _HtmlImportScreenState extends State<HtmlImportScreen> {
  ...
  Future<void> _kaydet() async {
    ...
    for (final s in secilenler) {
      try { await _db.ekle(s.toPolice()); basarili++; } catch (_) {}
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text('$basarili poliçe'
          '${_zeyiller.isNotEmpty ? ' · ${_zeyiller.length} zeyil' : ''} aktarıldı'),
                    ...
                    if (s.policeNo.isNotEmpty || s.belgeSeriNo.isNotEmpty)
                      Row(children: [
                        if (s.policeNo.isNotEmpty) ...[
                          ...
                          Flexible(child: Text('Poliçe: ${s.policeNo}',
                              style: const TextStyle(fontSize: 11, color: kTextSub),
                              overflow: TextOverflow.ellipsis)),
                          const SizedBox(width: 10),
                        ],
                        if (s.belgeSeriNo.isNotEmpty) ...[
                          ...
                          const SizedBox(width: 3),
                          Flexible(child: Text('Ruhsat: ${s.belgeSeriNo}',
                              style: const TextStyle(fontSize: 11, color: kTextSub),
                              overflow: TextOverflow.ellipsis)),
                        ],
                      ]),
