...
class _Satir {
  ...
  Police toPolice() {
    ...
    // Grup kişi sayısı ve bilgilerini hazırla
    int? kisiSayisi;
    List<GrupKisi>? grupKisileriList;
    if (grupKisileri != null && grupKisileri!.isNotEmpty) {
      kisiSayisi = grupKisileri!.length;
      grupKisileriList = grupKisileri!.map((k) => GrupKisi(
        ad: k.ad,
        soyad: k.soyad,
        tcKimlikNo: k.tcKimlik.isEmpty ? null : k.tcKimlik,
      )).toList();
    }
          ...
          for (final kisi in s.grupKisileri!) {
            ...
            if (!basGun.isAfter(bugun)) {
              ...
              final policeBaslangic = Police(
                ...
                durum:           PoliceStatus.yapildi, // Başlangıç → Yapıldı
              );
              await _db.ekleVeyaGuncelle(policeBaslangic);
              basarili++;
            }
            
            // BİTİŞ KAYDI - sadece bitiş tarihi gelecekteyse ve başlangıçtan farklı ay/yılsa
            if (bitGun.isAfter(bugun) && 
                (bit.year != bas.year || bit.month != bas.month)) {
              final belgeSeri = (s.tur == PoliceType.kasko || s.tur == PoliceType.trafik) 
                  ? s.belgeSeriNo 
                  : null;
              
              final policeBitis = Police(
                musteriAdi:      kisi.ad,
                soyadi:          kisi.soyad,
                telefon:         '-',
                tcKimlikNo:      kisi.tcKimlik.isEmpty ? null : kisi.tcKimlik,
                ...
                tutar:           s.tutar / s.grupKisileri!.length,
                komisyon:        s.komisyon / s.grupKisileri!.length,
                durum:           PoliceStatus.beklemede, // Bitiş → Beklemede
              );
              await _db.ekleVeyaGuncelle(policeBitis);
              basarili++;
            }
          }
        } else {
