...
class _Satir {
  ...
  Police toPolice() {
    ...
    // Belge Seri No sadece Kasko ve Trafik için
    final belgeSeri = (tur == PoliceType.kasko || tur == PoliceType.trafik) 
        ? belgeSeriNo 
        : null;
    ...
    return Police(
      ...
      sirket:          sirket,
      tur:             tur,
      policeNo:        policeNo.isEmpty ? null : policeNo,  // HTML'deki Police No → Poliçe Numarası
      belgeSeriNo:     belgeSeri, // Sadece Kasko/Trafik için
      ruhsatSeriNo:    null, // HTML'de Ruhsat Seri No yok
...
enum _ImportModu { baslangic, bitis }
...
class _HtmlImportScreenState extends State<HtmlImportScreen> {
  ...
  _ImportModu      _importModu   = _ImportModu.baslangic;

  Future<void> _dosyaSec({required _ImportModu mod}) async {
    setState(() => _importModu = mod);
    ...
    try {
      ...
      final satirlar = result.$1;
      // Bitiş modunda hiçbir şey seçili gelmesin
      if (mod == _ImportModu.bitis) {
        for (final s in satirlar) { s.secili = false; }
      }
      setState(() {
        _satirlar   = satirlar;
        _zeyiller   = mod == _ImportModu.bitis ? [] : result.$2;
        _dosyaAdi   = file.name;
        _yukleniyor = false;
      });
    } catch (e) {
  ...
  (List<_Satir>, List<ZeyilKayit>) _parseHtml(String html) {
    ...
    for (int r = 1; r < enIyi.length; r++) {
      ...
      final tur = PoliceTypeX.fromExcel(policeStr);
      final belgeSeri = (tur == PoliceType.kasko || tur == PoliceType.trafik) 
          ? get(row, iBelgeSeri) 
          : '';
    ...
    for (final s in secilenler) {
      try {
        if (s.isGrupBasligi && s.grupKisileri != null && s.grupKisileri!.isNotEmpty) {
          // Grup poliçesi - her kişi için kayıt oluştur
          for (final kisi in s.grupKisileri!) {
            final bas = s.baslangic ?? DateTime.now();
            final bit = s.bitis ?? bas.add(const Duration(days: 365));
            final basGun = DateTime(bas.year, bas.month, bas.day);
            
            // BAŞLANGIÇ KAYDI - sadece başlangıç tarihi geçmiş/bugünse
            if (!basGun.isAfter(bugun)) {
              final belgeSeri = (s.tur == PoliceType.kasko || s.tur == PoliceType.trafik) 
                  ? s.belgeSeriNo 
                  : null;
              
              final policeBaslangic = Police(
                musteriAdi:      kisi.ad,
                soyadi:          kisi.soyad,
                telefon:         '-',
                ...
                sirket:          s.sirket,
                tur:             s.tur,
                policeNo:        s.policeNo.isEmpty ? null : s.policeNo,
                belgeSeriNo:     belgeSeri,
                ruhsatSeriNo:    null,
                aracPlaka:       s.plaka.isEmpty ? null : s.plaka,
                baslangicTarihi: bas,
                bitisTarihi:     bit,
                tutar:           s.tutar / s.grupKisileri!.length,
                komisyon:        s.komisyon / s.grupKisileri!.length,
                durum:           _importModu == _ImportModu.bitis
                                   ? PoliceStatus.beklemede
                                   : PoliceStatus.yapildi,
              );
              await _db.ekleVeyaGuncelle(policeBaslangic);
              basarili++;
            }
            
          }
        } else {
          // Tekil poliçe - aynı mantık
          final bas = s.baslangic ?? DateTime.now();
          final bit = s.bitis ?? bas.add(const Duration(days: 365));
          final basGun = DateTime(bas.year, bas.month, bas.day);
          ...
          if (!basGun.isAfter(bugun)) {
            ...
            final policeBaslangic = Police(
              ...
              soyadi:          s.soyadi,
              telefon:         '-',
              tcKimlikNo:      s.tcKimlik.isEmpty ? null : s.tcKimlik,
              sirket:          s.sirket,
              tur:             s.tur,
              policeNo:        s.policeNo.isEmpty ? null : s.policeNo,
              belgeSeriNo:     belgeSeri,
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Row(children: [
                ...
                SizedBox(width: 6),
                Text('Uyarılar', style: TextStyle(fontWeight: FontWeight.w800, color: Colors.red)),
              ]),
        ...
        if (_satirlar.isEmpty && !_yukleniyor)
          Expanded(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const Text(
              ...
              style: TextStyle(fontSize: 12, color: Colors.blackSub, height: 1.5),
            ),
            ...
            // Başlangıç Yükle
            ElevatedButton.icon(
              onPressed: () => _dosyaSec(mod: _ImportModu.baslangic),
              icon: const Icon(Icons.upload_file_rounded),
              label: const Text('Başlangıç Yükle'),
            ...
            const SizedBox(height: 6),
            const Text('Yeni poliçeler — otomatik Yapıldı',
                style: TextStyle(fontSize: 11, color: Colors.blackSub)),
            const SizedBox(height: 16),
            // Bitiş Yükle
            ElevatedButton.icon(
              onPressed: () => _dosyaSec(mod: _ImportModu.bitis),
              icon: const Icon(Icons.event_rounded),
              label: const Text('Bitiş Yükle'),
              style: ElevatedButton.styleFrom(
            ...
            const SizedBox(height: 6),
            const Text('Bitiş hatırlatma — seçsiz gelir, sen seçersin',
                style: TextStyle(fontSize: 11, color: Colors.blackSub)),
          ]))),
        ...
        if (_satirlar.isNotEmpty) ...[
          ...
          Container(
            ...
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _importModu == _ImportModu.baslangic
                          ? const Color(0xFF1565C0).withOpacity(.1)
                          : const Color(0xFF6A1B9A).withOpacity(.1),
                      borderRadius: BorderRadius.circular(6),
                    ),
                    child: Text(
                      _importModu == _ImportModu.baslangic ? '🟦 Başlangıç' : '🟣 Bitiş',
                ...
                Row(children: [
                  Text('${_satirlar.length} poliçe · $secili seçili',
                      style: const TextStyle(fontSize: 12, color: Colors.blackSub)),
                  if (_zeyiller.isNotEmpty) ...[
              ...
              TextButton(
                onPressed: () => _dosyaSec(mod: _importModu),
                child: const Text('Değiştir', style: TextStyle(color: Colors.blue)),
              ),
            ]),
          ),
            ...
            itemBuilder: (_, i) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: s.hata != null ? Colors.red.withOpacity(0.4) : Colors.grey,
                      ...
                      title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(children: [
                          if (s.sirket.isNotEmpty) ...[
                            const Icon(Icons.business_rounded, size: 11, color: Colors.blackSub),
                            const SizedBox(width: 4),
                            Text(s.sirket,
                                style: const TextStyle(fontSize: 11.5, color: Colors.blackSub,
                                    fontWeight: FontWeight.w600)),
                            const SizedBox(width: 10),
                          ],
                          if (s.tcKimlik.isNotEmpty && !s.isGrupBasligi) ...[
                            const Icon(Icons.badge_outlined, size: 11, color: Colors.blackSub),
                            const SizedBox(width: 4),
                            Expanded(child: Text(s.tcKimlik,
                                style: const TextStyle(fontSize: 11, color: Colors.blackSub),
                                overflow: TextOverflow.ellipsis)),
                          ],
                        ]),
                        ...
                        if (s.policeNo.isNotEmpty || s.belgeSeriNo.isNotEmpty)
                          Row(children: [
                            if (s.policeNo.isNotEmpty) ...[
                              const Icon(Icons.numbers_rounded, size: 11, color: Colors.blackSub),
                            ...
                            if (s.belgeSeriNo.isNotEmpty) ...[
                              const Icon(Icons.confirmation_number_outlined, size: 11, color: Colors.blackSub),
                              const SizedBox(width: 3),
                        ...
                        Row(children: [
                          ...
                          if (s.baslangic != null && s.bitis != null)
                            const Padding(
                              ...
                              child: Icon(Icons.arrow_forward_rounded, size: 11, color: Colors.blackSub),
                            ),
                          if (s.bitis != null)
      ...
      bottomNavigationBar: _satirlar.isNotEmpty
          ? SafeArea(child: Padding(
            ...
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: _importModu == _ImportModu.baslangic
              ...
              child: _kaydediliyor
                  ...
                  : Text(
                      _importModu == _ImportModu.baslangic
                          ? '$secili Poliçeyi Aktar (Yapıldı)'
                          : '$secili Poliçeyi Aktar (Beklemede)',
