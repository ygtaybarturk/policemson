// lib/screens/analizler_screen.dart – v5 clean
...
class _AnalizlerScreenState extends State<AnalizlerScreen>
  ...
  Future<void> _yukle() async {
    ...
    try {
      ...
      // bas: en eski ay, bit: şu anki ay
      int bas = simdiAy - _donem + 1;
      int bit = simdiAy;
      int yil = _yil;
      // Eğer bas 0 veya negatif olursa önceki yıla geç — ama basit tutalım
      if (bas < 1) bas = 1;
      final v = await _db.aralikAnaliz(yil, bas, bit);
  ...
  Map<String, int> get _turDagilimi {
    final map = <String, int>{};
    for (final r in _veri) {
      final td = r['turDagilimi'] as Map<String, int>?;
      td?.forEach((k, v) => map[k] = (map[k] ?? 0) + v);
    }
    return map;
  }

  final _fmt  = NumberFormat('#,##0.##', 'tr');
  final _para = NumberFormat.currency(locale: 'tr', symbol: '₺', decimalDigits: 2);
      ...
      body: Column(children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const Spacer(),
            Text('$_yil', style: const TextStyle(fontWeight: FontWeight.w800, color: kPrimary)),
          ]),
        ),
        Expanded(
          child: _yukleniyor
  ...
  Widget _bosEkran() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    Text('Bu dönem için poliçe bulunamadı',
  ...
  Widget _ozetTab() {
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      // ── Prim & Komisyon ──
      Row(children: [
        Expanded(child: _paraKart('💰 Toplam Prim', _toplamGelir, Colors.orange)),
        const SizedBox(width: 10),
        Expanded(child: _paraKart('📈 Komisyon', _toplamKomisyon, Colors.purple)),
      ]),
  ...
  Widget _toplamSatir() => Container(
    ...
    child: Row(children: [
      ...
      _h('₺${_fmt.format(_toplamGelir)}',    bold: true, renk: Colors.orange.shade700, flex: 3),
      _h('₺${_fmt.format(_toplamKomisyon)}', bold: true, renk: Colors.purple.shade700, flex: 3),
    ]),
  );
  ...
  // ══════════════════════════════════════════════════════════
  Widget _uretimTab() {
    if (_veri.isEmpty) return _bosEkran();
    return ListView(padding: const EdgeInsets.all(14), children: [

      // Poliçe üretim bar grafiği
            ...
            bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true,
                getTitlesWidget: (v, _) {
                  ...
                  return Padding(padding: const EdgeInsets.only(top: 4),
                      child: Text(_ayAdlari[_veri[i]['ay'] as int], style: const TextStyle(fontSize: 9)));
                })),
      ...
      // Gelir çizgi grafiği
      _kart('💰 Aylık Prim Geliri', SizedBox(
        height: 200,
        child: LineChart(LineChartData(
          lineBarsData: [
            ...
            LineChartBarData(
              spots: _veri.asMap().entries.map((e) =>
                  FlSpot(e.key.toDouble(), ((e.value['komisyon'] as double?) ?? 0.0) / 1000)).toList(),
              isCurved: true,
  ...
  Widget _portfoyTab() {
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      // Bar listesi
      _kart('📋 Türe Göre Adet & Oran', Column(
        children: sirali.asMap().entries.map((e) {
          final renk   = renkler[e.key % renkler.length];
          final yuzde  = toplam > 0 ? e.value.value / toplam : 0.0;
          return Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                const Spacer(),
                Text('${e.value.value} adet  %${(yuzde * 100).toStringAsFixed(1)}',
  ...
  // YARDIMCILAR
  // ══════════════════════════════════════════════════════════
  Widget _paraKart(String baslik, double tutar, Color renk) => Container(
    padding: const EdgeInsets.all(14),
    decoration: BoxDecoration(
