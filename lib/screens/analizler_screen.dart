// lib/screens/analizler_screen.dart – v9
...
class _AnalizlerScreenState extends State<AnalizlerScreen>
  ...
  // ── Yıl seçimi ──
  final int _minYil = 2020;
  final int _maxYil = 2030;
  late int _seciliYil;
  // Karşılaştırma için ikinci yıl (null = kapalı)
  int? _karsilastirYil;

  // Veri
  Map<String, dynamic> _veri      = {};
  Map<String, dynamic> _veri2     = {}; // karşılaştırma yılı
  bool _yukleniyor = true;

  // Seçili ay (drill-down için)
  int? _acikAy;
  ...
  Future<void> _yukle() async {
    ...
    try {
      final v1 = await _db.yillikOzet(_seciliYil);
      Map<String, dynamic> v2 = {};
      if (_karsilastirYil != null) {
  ...
  static const _ayTamAdlari = ['','Ocak','Şubat','Mart','Nisan','Mayıs','Haziran',
        ...
        Row(children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Row(
                children: List.generate(_maxYil - _minYil + 1, (i) {
                  final y = _minYil + i;
                  final sec = y == _seciliYil;
                  return Padding(
                    padding: const EdgeInsets.only(right: 6),
                    child: GestureDetector(
                      onTap: () { setState(() { _seciliYil = y; if (_karsilastirYil == y) _karsilastirYil = null; }); _yukle(); },
                      child: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
                        decoration: BoxDecoration(
                          color: sec ? kPrimary : kBg,
                          borderRadius: BorderRadius.circular(20),
                          border: Border.all(color: sec ? kPrimary : kBorder),
                        ),
                        child: Text('$y',
                  ...
                  Padding(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: _karsilastirYil == null ? Colors.grey.shade600 : kBg,
                          borderRadius: BorderRadius.circular(16),
                          border: Border.all(color: _karsilastirYil == null ? Colors.grey.shade600 : kBorder),
                        ),
                        child: Text('Yok',
                            style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                                color: _karsilastirYil == null ? Colors.white : kTextSub)),
                      ),
                    ),
                  ),
                    ...
                    return Padding(
                      ...
                      child: GestureDetector(
                        ...
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                          decoration: BoxDecoration(
                            color: sec ? Colors.orange.shade600 : kBg,
                            borderRadius: BorderRadius.circular(16),
                            border: Border.all(color: sec ? Colors.orange.shade600 : kBorder),
                          ),
                          child: Text('$y',
                              style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                                  color: sec ? Colors.white : kText)),
  ...
  Widget _ozetTab() {
    ...
    final toplam2    = karVar ? _gi(_veri2, 'toplam')    : 0;
    final netGelir2  = karVar ? _g(_veri2,  'netGelir')  : 0.0;
    final netKomis2  = karVar ? _g(_veri2,  'netKomisyon') : 0.0;
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      if (karVar) _karsilastirmaKart(
        ...
        toplam1: toplam, toplam2: toplam2,
        gelir1: netGelir, gelir2: netGelir2,
        komis1: netKomis, komis2: netKomis2,
      ),
      if (!karVar) ...[
        // ── Net Prim & Komisyon ──
        Row(children: [
        ...
        if (zeyilVar) _kart('🔄 Zeyil Özeti', _zeyilOzetWidget(
          ...
          zArtiAdet: _gi(_veri, 'zeyilArtiAdet'),
          zEksiAdet: _gi(_veri, 'zeyilEksiAdet'),
        )),
      ...
      // ── Aylık detay tablosu (tıklanabilir) ──
  ...
  Widget _karsilastirmaKart({
    ...
    Widget satir(String baslik, String v1, String v2, Color renk) => Padding(
      ...
      child: Row(children: [
        ...
        Expanded(flex: 2, child: Text(baslik,
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: kTextSub))),
        Expanded(child: Text(v2,
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900,
                color: renk.withOpacity(.6)))),
      ]),
    );

    final gelirFark = gelir1 - gelir2;
    final komisFark = komis1 - komis2;

    return _kart('📊 $_seciliYil vs $_karsilastirYil Karşılaştırma', Column(children: [
      // Başlık
      Row(children: [
      ...
      Row(children: [
        Expanded(child: Column(children: [
          ...
          Text('₺${_fmt.format(gelirFark.abs())}',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900,
                  color: gelirFark >= 0 ? Colors.green : Colors.red)),
          Text(gelirFark >= 0 ? '▲ Artış' : '▼ Azalış',
              style: TextStyle(fontSize: 10, color: gelirFark >= 0 ? Colors.green : Colors.red,
                  fontWeight: FontWeight.w700)),
        ])),
        Expanded(child: Column(children: [
          Text('Kom. Farkı', style: TextStyle(fontSize: 10, color: kTextSub)),
          Text('₺${_fmt.format(komisFark.abs())}',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900,
                  color: komisFark >= 0 ? Colors.green : Colors.red)),
          Text(komisFark >= 0 ? '▲ Artış' : '▼ Azalış',
              style: TextStyle(fontSize: 10, color: komisFark >= 0 ? Colors.green : Colors.red,
                  fontWeight: FontWeight.w700)),
        ])),
        Expanded(child: Column(children: [
          Text('Poliçe Farkı', style: TextStyle(fontSize: 10, color: kTextSub)),
          Text('${(toplam1 - toplam2).abs()}',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900,
                  color: toplam1 >= toplam2 ? Colors.green : Colors.red)),
          Text(toplam1 >= toplam2 ? '▲ Artış' : '▼ Azalış',
              style: TextStyle(fontSize: 10, color: toplam1 >= toplam2 ? Colors.green : Colors.red,
                  fontWeight: FontWeight.w700)),
        ])),
      ]),
    ]));
  ...
  Widget _zeyilOzetWidget({
    ...
    return Column(children: [
      satir(Icons.add_circle_outline, Colors.green.shade700,
          'Prim Zeyil+ (Ekleme)', zArtiAdet, zArti, zArtiKom),
      const SizedBox(height: 10),
      satir(Icons.remove_circle_outline, Colors.red.shade600,
          'İptal Zeyil- (Düşme)', zEksiAdet, zEksi, zEksiKom),
      const Divider(height: 16),
      Row(children: [
  ...
  // ── Aylık tablo (tıklanabilir, açılınca tur+zeyil detay) ──
  ...
  Widget _aylikSatirWidget(Map<String,dynamic> r) {
    ...
    final zeyilTutar = (r['zeyilTutar']  as double?) ?? 0.0;
    final zArti   = (r['zeyilArti']     as double?) ?? zeyilTutar.abs();
    final zEksi   = (r['zeyilEksi']     as double?) ?? (zeyilTutar < 0 ? zeyilTutar.abs() : 0.0);
    ...
    return Column(children: [
      ...
      InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            _hdr(_ayAdlari[ay], flex: 1, bold: true),
            _hdr('$top', flex: 1),
            _hdr(gelir > 0 ? '₺${_fmt.format(gelir)}' : '-', flex: 3,
                renk: Colors.orange.shade700),
            _hdr(komis > 0 ? '₺${_fmt.format(komis)}' : '-', flex: 3,
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (zeyilVar) ...[
              ...
              if (zArti > 0)
                _zeyilDetaySatir(
                    Icons.add_circle_outline, Colors.green.shade700,
                    'Prim Zeyil+', zArtiAdet, zArti,
                    (r['zeyilArtiKom'] as double?) ?? 0),
              if (zEksi > 0) ...[
                const SizedBox(height: 6),
                _zeyilDetaySatir(
                    Icons.remove_circle_outline, Colors.red.shade600,
                    'İptal Zeyil-', zEksiAdet, zEksi,
                    (r['zeyilEksiKom'] as double?) ?? 0),
              ],
              const SizedBox(height: 6),
              Row(children: [
  ...
  Widget _zeyilDetaySatir(IconData ikon, Color renk, String baslik,
      int adet, double tutar, double komis) =>
    Row(children: [
  ...
  Widget _uretimTab() {
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      _kart('📊 Aylık Poliçe Üretimi', SizedBox(
        ...
        child: BarChart(BarChartData(
          barGroups: List.generate(12, (i) {
            ...
            final yap  = (r['yapildi']    as int? ?? 0).toDouble();
            final yap2 = (r['yapilamadi'] as int? ?? 0).toDouble();
            return BarChartGroupData(x: i, barRods: [
              BarChartRodData(toY: yap,  color: Colors.green.shade600, width: 10,
                  borderRadius: BorderRadius.circular(3)),
              BarChartRodData(toY: yap2, color: Colors.red.shade400,   width: 10,
                  borderRadius: BorderRadius.circular(3)),
            ]);
          }),
          titlesData: FlTitlesData(
            bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true,
                getTitlesWidget: (v, _) => Padding(
                  ...
                  child: Text(_ayAdlari[v.toInt() + 1], style: const TextStyle(fontSize: 8))))),
      ...
      // ── Prim grafiği (+ karşılaştırma opsiyonel) ──
            ...
            lineBarsData: [
              LineChartBarData(
                spots: List.generate(12, (i) {
                  final ay = i + 1;
                  final r = vf.firstWhere((e) => e['ay'] == ay, orElse: () => {'netGelir': 0.0, 'gelir': 0.0, 'ay': ay});
                  final v = (r['netGelir'] as double?) ?? (r['gelir'] as double? ?? 0);
                  return FlSpot(i.toDouble(), v / 1000);
                }),
                isCurved: true,
                color: Colors.orange.shade700,
                barWidth: 3,
              ...
              if (karVar)
                LineChartBarData(
                  spots: List.generate(12, (i) {
                    final ay = i + 1;
                    final r = aylik2.firstWhere((e) => e['ay'] == ay,
                        orElse: () => {'netGelir': 0.0, 'gelir': 0.0, 'ay': ay});
                    final v = (r['netGelir'] as double?) ?? (r['gelir'] as double? ?? 0);
                    return FlSpot(i.toDouble(), v / 1000);
                  }),
                  isCurved: true,
      ...
      Row(children: [
        ...
        if (karVar) ...[
          const SizedBox(width: 12),
          _lejant(Colors.orange.shade300, '$_karsilastirYil Prim (K₺)'),
        ],
      ]),
      const SizedBox(height: 20),
    ]);
  ...
  // ══════════════════════════════════════════════════════════
  Widget _portfoyTab() {
    final td = _turD(_veri);
    final tp = _turP(_veri);
    final tk = _turK(_veri);
    final te = _turE(_veri);
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      // ── Yıllık toplam sayılar ──
      _kart('📋 $_seciliYil — Türe Göre Toplam Adet', Column(
        children: sirali.asMap().entries.map((e) {
          final renk  = _renkler[e.key % _renkler.length];
          final tur   = e.value.key;
          final adet  = e.value.value;
          final adet2 = td2[tur] ?? 0;
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
                    color: renk.withOpacity(.12), borderRadius: BorderRadius.circular(8)),
                ...
                if (karVar && adet2 > 0) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: Colors.orange.withOpacity(.12), borderRadius: BorderRadius.circular(8)),
                    child: Text('$_karsilastirYil: $adet2',
  ...
  Widget _bosEkran() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    const SizedBox(height: 12),
    Text('Bu yıl için poliçe bulunamadı', style: TextStyle(color: kTextSub, fontSize: 14)),
    const SizedBox(height: 8),
    Text('Farklı bir yıl seçin', style: TextStyle(color: kTextSub.withOpacity(.6), fontSize: 12)),
  ]));
