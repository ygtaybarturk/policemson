...
class _AnalizlerScreenState extends State<AnalizlerScreen>
  ...
  String get _paraFmt => NumberFormat.currency(locale:'tr',symbol:'₺',decimalDigits:2).format(_toplamGelir);
  String get _komisyonFmt => NumberFormat.currency(locale:'tr',symbol:'₺',decimalDigits:2).format(_toplamKomisyon);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Analizler', style: TextStyle(fontWeight:FontWeight.w800, fontSize:20, letterSpacing:-0.5, color:kText)),
        centerTitle: true,
        bottom: TabBar(
          controller: _tabCtrl,
          isScrollable: true,
  ...
  Widget _ozet() {
    ...
    return ListView(
      ...
      children: [
        ...
        // ── Ana Para Kartları ──
        Row(children: [
          Expanded(child: _buyukParaKarti('💰 Toplam Prim', _toplamGelir, Colors.orange)),
          const SizedBox(width: 10),
          Expanded(child: _buyukParaKarti('📈 Toplam Komisyon', _toplamKomisyon, Colors.purple)),
        ]),
        const SizedBox(height: 10),

        // ── Poliçe Sayı Kartları ──
        Row(children: [
          Expanded(child: _sayiKarti('📋 Toplam Poliçe', _toplamPolice, Colors.blue)),
          const SizedBox(width: 10),
  ...
  Widget _buyukParaKarti(String baslik, double tutar, Color renk) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(baslik, style: TextStyle(fontSize: 11, color: context.textSub, fontWeight: FontWeight.w700)),
  ...
  Widget _sayiKarti(String baslik, int sayi, Color renk) {
    return Container(
      ...
      child: Column(children: [
        Text('$sayi', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: renk)),
        const SizedBox(height: 2),
        Text(baslik, style: TextStyle(fontSize: 9.5, color: context.textSub, fontWeight: FontWeight.w600), textAlign: TextAlign.center),
      ]),
    );
  }
  ...
  Widget _aylikParaTablo() {
    ...
    final fmt = NumberFormat('#,##0.##', 'tr');
    double toplamPrim = 0, toplamKom = 0;
    for (final r in _veri) {
      toplamPrim += r['gelir'] as double;
      toplamKom  += (r['komisyon'] as double? ?? 0);
    }
    return Column(
      children: [
        // Başlık satırı
        ...
        ..._veri.map((r) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              border: Border(bottom: BorderSide(color: kDivider.withOpacity(.5))),
            ),
            child: Row(children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            _hucre('$_toplamPolice', bold: true),
            _hucre('₺${fmt.format(toplamPrim)}', bold: true, renk: Colors.orange.shade800, flex: 3),
            _hucre('₺${fmt.format(toplamKom)}', bold: true, renk: Colors.purple.shade700, flex: 3),
          ]),
        ),
      ],
    );
